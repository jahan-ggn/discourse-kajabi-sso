...
module KajabiSso
  module Infrastructure
    class KajabiAuthenticator
      TOKEN_PATH = "oauth/token"
      TOKEN_CACHE_BUFFER = 60

      def initialize(client_id:, client_secret:, base_uri:, transport:, cache:)
        @client_id = client_id
      ...
      def fetch_token
        ...
        response = with_circuit { @transport.request(uri, req) }
      ...
      def cache_key
        Infrastructure::CacheKeyBuilder.build("token", Digest::SHA256.hexdigest(@client_id))
      end

      def with_circuit
        CircuitBreaker.check
        yield.tap { CircuitBreaker.record_success }
